...
def main():
    ...
    cube0.spawn_id_degrees(actorNumber=10, location=[2.20, 0.812, 0.1], rotation=[0, 0, 0], scale=[0.15, 0.15, 0.2],
                        configuration=cube0.SHAPE_CUBE, waitForConfirmation=True)
...
class Lidar_detect:
    def __init__(self):
        self.lidar_rate = 0.01

    def lidar_detect(self,qcar,lock):
        lock.acquire()
        success, angle, distance = qcar.get_lidar(samplePoints=720)
        ...
        for label in unique_labels:
            ...
            if cluster_radius < 0.1:
                output_center = cluster_center
                output_radius = cluster_radius
        #     if label == -1:
        #         plt.scatter(cloud_data[labels == label, 0], cloud_data[labels == label, 1], color='gray', label='Noise')
        #     else:
        #         cluster_points = cloud_data[labels == label]
        ...
        # plt.ylabel('Y-axis')
        # plt.title('DBSCAN Clustering')
        # plt.pause(0.01)  # 等待0.1秒，确保图形有足够的时间显示
        # plt.clf()  # 清除当前图形，准备下一次迭代
