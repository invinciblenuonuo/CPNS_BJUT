...
#     parser.add_argument('--weights', nargs='+', type=str, default='YOLO/best.pt', help='model.pt path(s)')
#     parser.add_argument('--source', type=str, default='YOLO/dataset/train/images', help='source')  # file/folder, 0 for webcam
...
class detectapi:
    def __init__(self,weights,img_size=640):
        self.opt=yolo_opt(weights=weights,img_size=img_size)
    ...
    def detect(self,image):
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                plot_one_box(xyxy, image0, label=label, color=self.colors[int(cls)], line_thickness=3)
        end_time = time.time()
        runtime = "{:.3f}".format(end_time - start_time) + "s"
        cv2.imshow("Image", image0)
        cv2.waitKey(1)
        return result_txt, self.names , runtime

if __name__ == '__main__':
    detect_yolo = detectapi(weights='D:/Documentes/postgraduate/qcar/official/CPNS_BJUT/YOLO/best.pt')
    image1 = cv2.imread("D:/Documentes/postgraduate/qcar/official/CPNS_BJUT/YOLO/dataset/train/images/stop_sign_112.jpeg")
    result,label,runtime=detect_yolo.detect(image1)
